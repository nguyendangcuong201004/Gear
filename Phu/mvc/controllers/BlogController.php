    ...
    public function create() {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($blogModel->createPost($title, $category, $image, $content)) {
                echo "<script>alert('Post created successfully!'); window.location='/ltw/BlogController/list';</script>";
            } else {
    ...
    public function detail(...$params) {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'delete') {
            if ($blogModel->deletePostById($post_id)) {
                header("Location: /ltw/BlogController/list"); // hoặc route phù hợp
                exit;
            } else {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['name'], $_POST['email'], $_POST['comment'])) {
            ...
            if ($blogModel->createComment($post_id, $name, $email, $comment)) {
                header("Location: /ltw/BlogController/detail/" . $post_id);
                exit;
            } else {
